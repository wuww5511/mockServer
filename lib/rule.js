 ...
 *  - proxy {String} 
 */
module.exports.genRule = function (opts) {
    opts = opts || {}
    const rule = {
        beforeSendRequest: function (requestDetail) {
            var funcs = [useInclude, useExclude, useMock, useProxy]
            for (var i = 0; i < funcs.length; i++) {
                // 返回false表示插件不处理该请求
                var res = funcs[i].call(null, opts, requestDetail)
                if (res !== false) {
                    return res
                }
            }
...
function useMock (opts, requestDetail) {
    if (opts.mock) {
        delete require.cache[opts.mock]
        var mock = require(opts.mock)
